...
#define SLD_KPROP 0x024
#define DEV_NAME "sld,kprop"

/* <<--params-->> */
const int32_t input_dimension = 13;
const int32_t nodes_per_layer = 64;
const int32_t possible_outputs = 3;
const int32_t learning_rate = 0.01;
const int32_t training_sets = 163;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_KPROP, DEV_NAME);
	if (ndev == 0) {
#ifndef __riscv
		printf("kprop not found\n");
#else
		print_uart("kprop not found\n");
#endif
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
		...
		/* <<--regs-config-->> */
		iowrite32(dev, KPROP_INPUT_DIMENSION_REG, input_dimension);
		iowrite32(dev, KPROP_NODES_PER_LAYER_REG, nodes_per_layer);
		iowrite32(dev, KPROP_POSSIBLE_OUTPUTS_REG, possible_outputs);
		iowrite32(dev, KPROP_LEARNING_RATE_REG, learning_rate);
		iowrite32(dev, KPROP_TRAINING_SETS_REG, training_sets);
