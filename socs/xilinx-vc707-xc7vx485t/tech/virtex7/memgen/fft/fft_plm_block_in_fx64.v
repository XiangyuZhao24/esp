...
module fft_plm_block_in_fx64(
  ...
  reg               bank_CE  [0:0][0:0][1:0][63:0][1:0];
  reg        [13:0] bank_A   [0:0][0:0][1:0][63:0][1:0];
  reg         [0:0] bank_D   [0:0][0:0][1:0][63:0][1:0];
  reg               bank_WE  [0:0][0:0][1:0][63:0][1:0];
  reg         [0:0] bank_WEM [0:0][0:0][1:0][63:0][1:0];
  wire        [0:0] bank_Q   [0:0][0:0][1:0][63:0][1:0];
  wire        [0:0] ctrld    [3:2];
  wire        [0:0] ctrlh    [3:0];
  wire        [0:0] ctrlv    [3:0];
  reg         [0:0] seld     [3:2];
...
// synopsys translate_off
  integer check_bank_access [0:0][0:0][1:0][63:0][1:0];
...
// synthesis translate_on
  ...
  for (h = 0; h < 1; h = h + 1) begin : gen_ctrl_hbanks
    for (v = 0; v < 2; v = v + 1) begin : gen_ctrl_vbanks
      for (hh = 0; hh < 64; hh = hh + 1) begin : gen_ctrl_hhbanks
...
// synthesis translate_on
  ...
  generate
  for (hh = 0; hh < 64; hh = hh + 1) begin : gen_q_assign_hhbanks
    if (hh == 63 && (hh + 1) * 1 > 64) begin : gen_q_assign_hhbanks_last_2 
       assign Q2[63:1 * hh] = bank_Q[seld[2]][selh[2]][selv[2]][hh][0][0:0];
    end else begin : gen_q_assign_hhbanks_others_2 
      assign Q2[1 * (hh + 1) - 1:1 * hh] = bank_Q[seld[2]][selh[2]][selv[2]][hh][0];
    end
    if (hh == 63 && (hh + 1) * 1 > 64) begin : gen_q_assign_hhbanks_last_3 
       assign Q3[63:1 * hh] = bank_Q[seld[3]][selh[3]][selv[3]][hh][1][0:0];
  ...
  for (d = 0; d < 1; d = d + 1) begin : gen_wires_dbanks
    for (h = 0; h < 1; h = h + 1) begin : gen_wires_hbanks
      for (v = 0; v < 2; v = v + 1) begin : gen_wires_vbanks
        for (hh = 0; hh < 64; hh = hh + 1) begin : gen_wires_hhbanks
